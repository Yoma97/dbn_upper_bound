...
int main(void){
 ...
 /* Conservative powers valid for 0<t<=0.01, lambda>=6.90:
    u=t log n<=1: exponent >=1.9749 -> use s=1.974;
    u>1 through the moving cutoff: exponent >=1.3623 -> use q=1.362. */
 S(T,"0.01",MPFR_RNDU);S(s,"1.974",MPFR_RNDD);S(q,"1.362",MPFR_RNDD);U(tmp,2);mpfr_log(ln2,tmp,MPFR_RNDU);
 ...
 /* Deliberately padded Polymath error envelopes. */
 S(tmp,"6.90",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(eab,tmp,MPFR_RNDU);U(tmp,256);mpfr_mul(eab,eab,tmp,MPFR_RNDU);
 /* On the rho=0.1 disk the scaled W exceeds 6.8; W/4+W^2/16 >=4.59. */
 S(tmp,"4.59",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);U(tmp2,1);mpfr_add(tmp,tmp,tmp2,MPFR_RNDU);mpfr_exp(ec,tmp,MPFR_RNDU);
 /* The second error-series power is >1.312 and the disk cutoff has scaled log >=6.8/(2t); use the weaker 4.45 exponent. */
 S(tmp,"4.45",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(jump,tmp,MPFR_RNDU);U(tmp,8);mpfr_mul(jump,jump,tmp,MPFR_RNDU);mpfr_add(E0,eab,ec,MPFR_RNDU);mpfr_add(E0,E0,jump,MPFR_RNDU);
 /* Same symmetric-normalization envelope used for the full upper endpoint 10.52. */
