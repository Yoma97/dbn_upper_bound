...
int main(void){
 ...
 /* For 0<t<=0.01 and lambda>=6.50:
    u=t log n<=1 gives exponent >1.8749, use s=1.874;
    the remaining moving-cutoff region gives >1.3123, use q=1.311. */
 S(T,"0.01",MPFR_RNDU);S(s,"1.874",MPFR_RNDD);S(q,"1.311",MPFR_RNDD);U(tmp,2);mpfr_log(ln2,tmp,MPFR_RNDU);
 mpfr_mul(tmp,s,ln2,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(two_neg_s,tmp,MPFR_RNDU);U(tmp,1);mpfr_sub(sm1,s,tmp,MPFR_RNDD);
 ...
 U(tmp,1);mpfr_sub(sm1,q,tmp,MPFR_RNDD);U(tmp,1);mpfr_sub(tmp,tmp,q,MPFR_RNDU);U(tmp2,100);mpfr_mul(tmp,tmp,tmp2,MPFR_RNDU);mpfr_exp(E100,tmp,MPFR_RNDU);
 ...
 U(tmp,100);mpfr_mul(tmp2,q,tmp,MPFR_RNDD);NEG(tmp2,tmp2,MPFR_RNDU);mpfr_exp(A1t,tmp2,MPFR_RNDU);mpfr_mul(A1t,A1t,tmp,MPFR_RNDU);mpfr_div(tmp2,tmp,sm1,MPFR_RNDU);mpfr_sqr(tmp3,sm1,MPFR_RNDD);U(tmp,1);mpfr_div(tmp,tmp,tmp3,MPFR_RNDU);mpfr_add(tmp2,tmp2,tmp,MPFR_RNDU);mpfr_mul(tmp2,tmp2,E100,MPFR_RNDU);mpfr_add(A1t,A1t,tmp2,MPFR_RNDU);
 mpfr_add(A0,A0h,A0t,MPFR_RNDU);mpfr_add(A1,A1h,A1t,MPFR_RNDU);U(tmp,1);mpfr_sub(S0,tmp,A0,MPFR_RNDD);
 /* Deliberately padded unconditional error envelopes. */
 S(tmp,"6.50",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(eab,tmp,MPFR_RNDU);U(tmp,2048);mpfr_mul(eab,eab,tmp,MPFR_RNDU);
 /* The scaled disk variable exceeds 6.4; use W/4+W^2/16>4.10. */
 S(tmp,"4.10",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);U(tmp2,1);mpfr_add(tmp,tmp,tmp2,MPFR_RNDU);mpfr_exp(ec,tmp,MPFR_RNDU);
 /* Worst positive error-series power stays >1.262 and log cutoff >3.2/t;
    use the weaker jump exponent 3.95/t. */
 S(tmp,"3.95",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(jump,tmp,MPFR_RNDU);U(tmp,8);mpfr_mul(jump,jump,tmp,MPFR_RNDU);mpfr_add(E0,eab,ec,MPFR_RNDU);mpfr_add(E0,E0,jump,MPFR_RNDU);
 ...
 /* L>=650 so Phi=L/4+o(1)>162.49. */
 S(phi,"162.49",MPFR_RNDD);S(d,"0.501",MPFR_RNDU);U(tmp,2);mpfr_div(half,E0,tmp,MPFR_RNDU);mpfr_sqr(rad,S0,MPFR_RNDD);mpfr_sqr(tmp,half,MPFR_RNDU);mpfr_sub(rad,rad,tmp,MPFR_RNDD);mpfr_sqrt(Y,rad,MPFR_RNDD);mpfr_mul(core,phi,Y,MPFR_RNDD);mpfr_mul(tmp,d,A1,MPFR_RNDU);mpfr_sub(core,core,tmp,MPFR_RNDD);U(tmp,2);mpfr_mul(lhs,core,tmp,MPFR_RNDD);mpfr_sub(margin,lhs,E1,MPFR_RNDD);
 printf("STRICT SMALL-TIME lambda>=6.50 audit PREC=%d\n",PREC);P("A0_upper",A0,MPFR_RNDU);P("A1_upper",A1,MPFR_RNDU);P("S0_lower",S0,MPFR_RNDD);P("E0_upper",E0,MPFR_RNDU);P("E1_upper",E1,MPFR_RNDU);P("lhs_lower",lhs,MPFR_RNDD);P("margin_lower",margin,MPFR_RNDD);int ok=mpfr_cmp_ui(margin,0)>0;printf("AUDIT_RESULT pass=%d\n",ok);for(size_t i=0;i<sizeof(v)/sizeof(v[0]);i++)mpfr_clear(*v[i]);return ok?0:1;
}
