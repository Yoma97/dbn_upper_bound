...
int main(void){
 ...
 /* For 0<t<=0.01 and lambda>=6.83:
    if u=t log n<=1 then exponent > 1.9574, use s=1.957;
    through the remaining moving cutoff exponent >1.35355, use q=1.353. */
 S(T,"0.01",MPFR_RNDU);S(s,"1.957",MPFR_RNDD);S(q,"1.353",MPFR_RNDD);U(tmp,2);mpfr_log(ln2,tmp,MPFR_RNDU);
 ...
 /* Deliberately padded Polymath remainder envelopes. */
 S(tmp,"6.83",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(eab,tmp,MPFR_RNDU);U(tmp,1024);mpfr_mul(eab,eab,tmp,MPFR_RNDU);
 /* The scaled disk variable exceeds 6.7; use W/4+W^2/16 >4.48. */
 ...
 /* Worst upper-semicircle series power is >1.3035.  With scaled log cutoff >6.7/2,
    the true jump exponent is >4.36/t; use 4.33/t with prefactor 8. */
 S(tmp,"4.33",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(jump,tmp,MPFR_RNDU);U(tmp,8);mpfr_mul(jump,jump,tmp,MPFR_RNDU);mpfr_add(E0,eab,ec,MPFR_RNDU);mpfr_add(E0,E0,jump,MPFR_RNDU);
 /* Same normalization envelope already audited over the full lambda<=10.52 range. */
