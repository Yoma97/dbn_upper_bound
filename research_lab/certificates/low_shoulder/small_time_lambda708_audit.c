...
int main(void){
 ...
 S(T,"0.01",MPFR_RNDU);S(s,"2.019",MPFR_RNDD);S(q,"1.384",MPFR_RNDD);U(tmp,2);mpfr_log(ln2,tmp,MPFR_RNDU);
 ...
 S(tmp,"7.08",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(eab,tmp,MPFR_RNDU);U(tmp,128);mpfr_mul(eab,eab,tmp,MPFR_RNDU);
 S(tmp,"4.8125",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);U(tmp2,1);mpfr_add(tmp,tmp,tmp2,MPFR_RNDU);mpfr_exp(ec,tmp,MPFR_RNDU);
 S(tmp,"4.72236",MPFR_RNDD);mpfr_div(tmp,tmp,T,MPFR_RNDD);NEG(tmp,tmp,MPFR_RNDU);mpfr_exp(jump,tmp,MPFR_RNDU);U(tmp,6);mpfr_mul(jump,jump,tmp,MPFR_RNDU);mpfr_add(E0,eab,ec,MPFR_RNDU);mpfr_add(E0,E0,jump,MPFR_RNDU);
